...
struct drawable {
    ...
    drawable(sf::Vector2f size = sf::Vector2f(50.0f, 50.0f), sf::Color color = sf::Color::White)
    {
        shape.setSize(size);
...
struct controllable {
    ...
    controllable(float speed = 200.0f) : speed(speed) {}
};
...
struct acceleration {
    ...
    acceleration(float ax = 0.0f, float ay = 0.0f, float max_speed = 300.0f, float friction = 0.95f)
...
struct looping {
    ...
    looping(float width = 800.0f, float height = 600.0f)
