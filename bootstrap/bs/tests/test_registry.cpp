...
void test_spawn_entities() {
	...
	std::cout << "✓ spawn_entity works (entities: " << entity0 << ", " << entity1 << ", " << entity2 << ")" << std::endl;
}
...
void test_register_and_get_components() {
	...
	// Get components (should return same references)
...
void test_add_component() {
	...
	assert(positions[player].has_value());
	assert(positions[player]->x == 10.0f);
	assert(positions[enemy].has_value());
	assert(positions[enemy]->x == 100.0f);
	
	auto& velocities = reg.get_components<Velocity>();
	assert(velocities[player].has_value());
	assert(velocities[player]->dx == 1.0f);
	
	auto& healths = reg.get_components<Health>();
	assert(healths[enemy].has_value());
	assert(healths[enemy]->hp == 50);
...
void test_emplace_component() {
	...
	assert(positions[entity]->x == 50.0f);
	assert(positions[entity]->y == 60.0f);
	
	auto& names = reg.get_components<Name>();
	assert(names[entity].has_value());
	assert(names[entity]->value == "Player");
...
void test_ecs_system() {
	...
	// Verify results
	assert(positions[player]->x == 5.0f);
	assert(positions[player]->y == 3.0f);
	
	assert(positions[enemy1]->x == 98.0f);
	assert(positions[enemy1]->y == 99.0f);
	
	assert(positions[enemy2]->x == 201.0f);
	assert(positions[enemy2]->y == 201.0f);
	
	assert(positions[static_obj]->x == 50.0f); // Unchanged
	assert(positions[static_obj]->y == 50.0f);
	
	std::cout << "✓ Movement system works correctly" << std::endl;
	std::cout << "  Player moved to (" << positions[player]->x << ", " << positions[player]->y << ")" << std::endl;
	std::cout << "  Static object stayed at (" << positions[static_obj]->x << ", " << positions[static_obj]->y << ")" << std::endl;
}
...
void test_auto_register() {
	...
	// get_components should auto-register if not already registered
	auto& positions = reg.get_components<Position>();
	
	positions.insert_at(entity, Position(42.0f, 24.0f));
	
	assert(reg.has_component<Position>(entity));
	assert(positions[entity]->x == 42.0f);
