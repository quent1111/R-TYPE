...
void Game::broadcast_player_positions(UDPServer& server) {
    ...
    data.push_back(0x42);
    data.push_back(0xB5);
    data.push_back(0x01); // msg type: player_positions pour quand pacaud aura fait le protocole
...
void Game::runGameLoop(UDPServer& server) {
    ...
    double position_broadcast_accumulator = 0.0;
    double cleanup_accumulator = 0.0;
    const double position_broadcast_interval = 0.1; // Positions toute les 100ms
    ...
    while (server_running) {
        ...
        while (lag >= fixed_timestep) {
            ...
            while (server.get_input_packet(packet)) {
                if (!packet.data.empty()) {
                    ...
                    if (!player_opt.has_value()) {
                        float start_x = 100.0f + (client_id * 50.0f);
                        ...
                        NetworkPacket welcome_packet(welcome_data, packet.sender);
            ...
            // Envoyer les positions des joueurs toutes les 100ms
            position_broadcast_accumulator += dt;
            if (position_broadcast_accumulator >= position_broadcast_interval) {
                broadcast_player_positions(server);
                position_broadcast_accumulator -= position_broadcast_interval;
            }
