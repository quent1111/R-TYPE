...
const std::array<std::array<float, 3>, 3> ColorTransform::PROTANOPIA_MATRIX = {
    {{0.567f, 0.433f, 0.0f}, {0.558f, 0.442f, 0.0f}, {0.0f, 0.242f, 0.758f}}};
...
const std::array<std::array<float, 3>, 3> ColorTransform::TRITANOPIA_MATRIX = {
    {{0.95f, 0.05f, 0.0f}, {0.0f, 0.433f, 0.567f}, {0.0f, 0.475f, 0.525f}}};
...
sf::Color ColorTransform::transform(const sf::Color& original, ColorBlindnessMode mode) {
    ...
    if (mode == ColorBlindnessMode::HighContrast) {
        ...
        return sf::Color(static_cast<sf::Uint8>(r * 255), static_cast<sf::Uint8>(g * 255),
                         static_cast<sf::Uint8>(b * 255), original.a);
    }
...
bool ColorTransform::areColorsDistinguishable(const sf::Color& color1, const sf::Color& color2,
                                              ColorBlindnessMode mode, float threshold) {
...
sf::Color ColorTransform::enhanceContrast(const sf::Color& foreground, const sf::Color& background,
                                          float factor) {
    ...
    return sf::Color(static_cast<sf::Uint8>(std::clamp(foreground.r * lum_ratio, 0.0f, 255.0f)),
...
std::array<float, 3> ColorTransform::rgbToLinear(const sf::Color& color) {
    ...
    return {toLinear(static_cast<float>(color.r)), toLinear(static_cast<float>(color.g)),
...
sf::Color ColorTransform::linearToRgb(const std::array<float, 3>& linear) {
    ...
    return sf::Color(static_cast<sf::Uint8>(std::clamp(toGamma(linear[0]) * 255.0f, 0.0f, 255.0f)),
...
std::array<float, 3>
ColorTransform::applyMatrix(const std::array<float, 3>& rgb,
                            const std::array<std::array<float, 3>, 3>& matrix) {
    return {matrix[0][0] * rgb[0] + matrix[0][1] * rgb[1] + matrix[0][2] * rgb[2],
