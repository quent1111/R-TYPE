...
LobbyListState::LobbyListState(sf::RenderWindow& window,
    ...
    : m_window(window), m_game_to_network_queue(game_to_net), m_network_to_game_queue(net_to_game) {
...
void LobbyListState::setup_ui() {
    ...
    m_title = std::make_unique<ui::MenuTitle>(
        "LOBBIES", sf::Vector2f(static_cast<float>(window_size.x) / 2.0f, 50.0f));
    ...
    auto play_btn = std::make_unique<ui::Button>(sf::Vector2f(play_button_x, bottom_y - 80.0f),
                                                 sf::Vector2f(play_button_width, 60.0f), "PLAY");
    ...
    auto create_btn = std::make_unique<ui::Button>(
        sf::Vector2f(start_x, bottom_y), sf::Vector2f(button_width, button_height), "CREATE LOBBY");
    create_btn->set_callback([this]() { on_create_clicked(); });
    m_buttons.push_back(std::move(create_btn));

    auto refresh_btn =
        std::make_unique<ui::Button>(sf::Vector2f(start_x + button_width + 20.0f, bottom_y),
                                     sf::Vector2f(button_width, button_height), "REFRESH");
    refresh_btn->set_callback([this]() { on_refresh_clicked(); });
    m_buttons.push_back(std::move(refresh_btn));

    auto back_btn =
        std::make_unique<ui::Button>(sf::Vector2f(start_x + (button_width + 20.0f) * 2, bottom_y),
    ...
    m_info_text.setPosition(static_cast<float>(window_size.x) / 2.0f - text_bounds.width / 2.0f,
                            200.0f);
}
...
void LobbyListState::on_play_clicked() {
    ...
    for (const auto& lobby : m_lobbies) {
        if (lobby.state_text == "Waiting" && lobby.current_players < lobby.max_players) {
            ...
            std::cout << "[LobbyListState] Found available lobby: " << lobby.name << " ("
                      << lobby.current_players << "/" << lobby.max_players << ")" << std::endl;
            break;
        }
        if (lobby.state_text == "InGame" && lobby.current_players < lobby.max_players &&
            ingame_lobby_id == -1) {
    ...
    std::string auto_lobby_name =
        "Game " + std::to_string(static_cast<int>(std::time(nullptr)) % 10000);
...
void LobbyListState::on_create_clicked() {
    ...
    m_input_text.setPosition(m_input_box.getPosition().x + 15.0f,
                             m_input_box.getPosition().y + 15.0f);
...
void LobbyListState::update_lobby_list_ui() {
    ...
    for (size_t i = 0; i < m_lobbies.size() && i < 6; ++i) {
        ...
        std::string button_text = lobby.name + "  [" + std::to_string(lobby.current_players) + "/" +
                                  std::to_string(lobby.max_players) + "]  " + lobby.state_text;

        auto lobby_btn = std::make_unique<ui::Button>(
            sf::Vector2f(button_x, y), sf::Vector2f(button_width, button_height), button_text);
        ...
        if (msg.type == NetworkToGame::MessageType::LobbyListUpdate &&
            ...
            if (opcode == RType::OpCode::ListLobbies) {
                ...
                for (int32_t i = 0; i < count; ++i) {
                    ...
                    switch (state) {
                        case 0:
                            info.state_text = "Waiting";
                            break;
                        case 1:
                            info.state_text = "Ready";
                            break;
                        case 2:
                            info.state_text = "In Game";
                            break;
                        case 3:
                            info.state_text = "Finished";
                            break;
                        default:
