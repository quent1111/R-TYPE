...
void LaserParticleSystem::spawn_glow_particles(float start_x, float start_y, float length) {
    ...
    for (int i = 0; i < num_glow; ++i) {
        ...
        particle.color.r = static_cast<sf::Uint8>(mid_color_.r * (1.0f - blend) + outer_color_.r * blend);
        particle.color.g = static_cast<sf::Uint8>(mid_color_.g * (1.0f - blend) + outer_color_.g * blend);
        particle.color.b = static_cast<sf::Uint8>(mid_color_.b * (1.0f - blend) + outer_color_.b * blend);
        particle.color.a = static_cast<sf::Uint8>(mid_color_.a * (1.0f - blend) + outer_color_.a * blend);
