            ...
            case NetworkToGame::MessageType::EntityUpdate: {
                ...
                for (const auto& p : msg.entities) {
                    ...
                    switch (incoming.type) {
                        case 0x01:
                            incoming.shape.setFillColor(sf::Color::Green);
                            break;
                        case 0x02:
                            incoming.shape.setFillColor(sf::Color::Red);
                            break;
                        case 0x03:
                            incoming.shape.setFillColor(sf::Color::Yellow);
                            break;
                        default:
                            incoming.shape.setFillColor(sf::Color::White);
                            break;
                    }
    ...
    for (auto& pair : entities_) {
        ...
        if (curr_t > prev_t) {
            ...
            if (alpha <= 0.0f) {
                ...
                const float after_ms =
                    std::chrono::duration_cast<std::chrono::duration<float>>(render_time - curr_t)
                        .count();
                if (after_ms > 0.0f) {
                    draw_x = e.x + (e.vx * after_ms);
                    draw_y = e.y + (e.vy * after_ms);
                } else {
                    draw_x = e.x;
                    draw_y = e.y;
                }
